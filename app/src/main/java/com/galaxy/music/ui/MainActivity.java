...
public class MainActivity extends Activity implements MusicService.ServiceCallback {
    ...
    private void applyScannedPlaylist(final ArrayList<Song> songs, int retryCount) {
        if (songs.size() == 0) return;
        if (!mBound) {
            // Service isn't bound yet (still starting up) - retry shortly instead of
            // silently dropping the scan result. Give up after ~5s so we don't loop forever.
            if (retryCount < 25) {
                final int nextRetry = retryCount + 1;
                mHandler.postDelayed(new Runnable() {
                    public void run() {
                        applyScannedPlaylist(songs, nextRetry);
                    }
                }, 200);
            }
            return;
